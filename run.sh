...
service_id=$(docker run -d -p 8080 -p 8081 --link $memcache_name:memcached benschw/service-discovery-links-example)
...
client_id=$(docker run -d -p 8080 -p 8081 --link $service_name:service benschw/service-discovery-links-example)
...
docker run -i -t hipache redis-cli -h $docker_bridge_ip -p 6379 rpush frontend:client.local client
docker run -i -t hipache redis-cli -h $docker_bridge_ip -p 6379 rpush frontend:client.local http://$docker_bridge_ip:$client_port
